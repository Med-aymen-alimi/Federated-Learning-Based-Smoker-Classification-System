...
Each client loads its local data subset and receives the current global model.

Clients perform local training and send updated weights to the server.

The server performs federated averaging of the weights from all clients.

The updated global model is sent back to clients.

Steps 2-5 repeat for a predefined number of federated rounds.

The final global model can be evaluated on test data.
...
Start the federated learning simulation using Docker Compose:
...
Monitor logs from each container to track training progress and weight updates.
